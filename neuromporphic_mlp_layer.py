...
class OrthonormalLinear(nn.Module):
    def __init__(self, num_groups, outputs_per_group, device=None, dtype=None):
        super(OrthonormalLinear, self).__init__()
        factory_kwargs = {'device': device, 'dtype': dtype}
        self.num_groups = num_groups
        self.outputs_per_group = outputs_per_group
        self.weight = Parameter(torch.empty((num_groups, num_groups, outputs_per_group, outputs_per_group), **factory_kwargs))
    ...
    def forward(self, x, orthoganal_weights, attention_mask=None):
        ...
        w = orthoganal_weights.reshape(self.num_groups*self.outputs_per_group, self.num_groups*self.outputs_per_group)
        out = torch.matmul(x_reshaped, w)
        return out.reshape(x.shape)

class NeuronalPerceptron(nn.Module):
    def __init__(self, inp_size, neurons, outputs_per_neuron):
        super(NeuronalPerceptron, self).__init__()
        self.neurons = neurons
        self.outputs_per_neuron = outputs_per_neuron
        self.ol = OrthonormalLinear(neurons, outputs_per_neuron)
        self.ln = nn.LayerNorm(neurons)
        self.il = nn.Linear(inp_size, neurons * outputs_per_neuron, bias=False)

    def get_weights(self):
        return self.ol.get_orthonormal_weights()

    def forward(self, x, weights, prev_state=0):
        x_combined = prev_state + self.il(x).reshape((-1, self.neurons, self.outputs_per_neuron)) / math.sqrt(self.outputs_per_neuron)
        ...
        new_mag = torch.sigmoid(self.ln(magnitudes[:,:,0]))
